...
export function ExampleUploader({ files, onChange }: ExampleUploaderProps) {
  ...
  const tooFew = files.length > 0 && files.length < 3;
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Sube entre 3 y 5 slides ejemplo (imagen o PDF).
      </p>
      <input
      ...
      {(tooFew || tooMany) && (
        <p className="mt-2 text-sm text-rose-600">Debes seleccionar entre 3 y 5 archivos.</p>
      )}
      {files.length > 0 && (
